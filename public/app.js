...
const validateSignal=price=>{if(pendingTest){pendingTest.ticksLeft--;if(pendingTest.ticksLeft===0){const won=pendingTest.direction==="RISE"?price>pendingTest.entry:price<pendingTest.entry;tests++;if(won)wins++;else losses++;pendingTest=null;updateScoreboard();}}const s=signal();if(!pendingTest&&(s==="RISE"||s==="FALL")&&s!==lastSignal){pendingTest={direction:s,entry:price,ticksLeft:Math.max(1,parseInt($("#duration").value,10)||5)};}lastSignal=s;};
